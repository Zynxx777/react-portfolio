...
export default function Box({img, gif, name, description, skills, link, subtitle, isOnline}){
    ...
    console.log('Available skills_imgs keys:', Object.keys(skills_imgs));
    ...
    >
      ...
      {skills.filter(skill => skill && skills_imgs[skill]).map((skill, index) => (
        <div
          key={index}
            ...
            <div className="w-6 h-6 bg-gray-400 rounded-full flex items-center justify-center text-xs font-bold text-white">
              ?
                ...
                {/* <div className="flex gap-4 mt-6">
                    {skills.map((skill) => skills_imgs[skill] ? <div key={skill} style={{backgroundImage: `url(${skills_imgs[skill].image})`}} className={`saturate-0 w-6 h-6 bg-center bg-no-repeat bg-contain`}  alt=""></div> : <div key={skill} className="w-6 h-6 bg-gray-400 rounded">?</div>
                    )}
                </div> */}
            </div>
